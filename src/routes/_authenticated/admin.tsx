    ...
    <div className="min-h-screen bg-background pb-20 md:pb-0">
      <header className="sticky top-0 z-30 border-b bg-background/85 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-6xl items-center gap-3 px-4">
          <Logo />
          <span className="ml-2 rounded-md bg-primary px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-primary-foreground">
            Admin
          </span>
            ...
            {NAV.map((n) => {
              ...
              return (
                <Link
                  key={n.to}
                  to={n.to as never}
                  className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-medium ${active ? "bg-secondary" : "text-muted-foreground hover:text-foreground"}`}
                >
        ...
        {NAV.map((n) => {
          ...
          return (
            <Link
              key={n.to}
              to={n.to as never}
              className={`flex flex-col items-center gap-0.5 py-2 text-[10px] font-medium ${active ? "text-primary font-bold" : "text-muted-foreground"}`}
            >
              <n.Icon className="h-4 w-4" />
