        ...
        <div className="mx-auto flex h-14 max-w-6xl items-center gap-3 px-4">
          ...
          <nav className="ml-4 hidden gap-1 md:flex">
            {[
              ...
              return (
                <Link
                  key={n.to}
                  to={n.to as never}
                  className={`rounded-lg px-3 py-1.5 text-sm font-medium ${active ? "bg-secondary text-secondary-foreground" : "text-muted-foreground hover:text-foreground"}`}
                >
                  {n.l}
                </Link>
              );
            ...
            {isAdmin && (
              <Link
                to="/admin"
                className={`rounded-lg px-3 py-1.5 text-sm font-medium ${path.startsWith("/admin") ? "bg-primary text-primary-foreground" : "text-primary hover:bg-primary/10"}`}
              >
                Admin
              </Link>
            )}
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="grid h-9 w-9 place-items-center rounded-full bg-hero text-sm font-bold text-primary-foreground ring-2 ring-primary/20">
                  {initial}
                </button>
              </DropdownMenuTrigger>
