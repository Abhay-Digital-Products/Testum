  ...
  useEffect(() => {
    (async () => {
      const [a, b, c, d, e, paidOrders, recentOrders, recentAttempts] = await Promise.all([
        ...
        supabase
          .from("orders")
          .select("id, plan_code, amount_inr, status, created_at, user_id")
          .order("created_at", { ascending: false })
          .limit(6),
        supabase
          .from("attempts")
          .select("id, score, status, submitted_at, tests(title)")
          .order("started_at", { ascending: false })
          .limit(6),
      ]);
      const revenue = (paidOrders.data ?? []).reduce(
        (t: number, o: any) => t + Number(o.amount_inr || 0),
        0,
      );
      setS({
        series: a.count ?? 0,
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Content, students and revenue at a glance.
        </p>
      </div>
          ...
          <div className="mt-3 space-y-2">
            ...
            {orders.map((o) => (
              ...
              >
                ...
                <span
                  className={`rounded-md px-2 py-0.5 text-[11px] font-semibold ${o.status === "paid" ? "bg-success/10 text-success" : "bg-secondary text-muted-foreground"}`}
                >
                  {o.status}
                </span>
              </div>
      ...
      <div className="rounded-2xl border bg-card p-5">
        ...
        <ol className="mt-3 list-decimal space-y-2 pl-5 text-sm">
          <li>
            Create a <b>Test Series</b> and link it to the plan that unlocks it.
          </li>
          <li>
            Add a <b>Test</b> with duration, marks and release date.
          </li>
          <li>
            Add <b>Questions</b> - paste question/option image URLs, mark the correct option, add a
            solution.
          </li>
          <li>
            Grant free access to a student any time from the <b>Students</b> page.
          </li>
        </ol>
        <Link to="/admin/tests" className="mt-4 inline-flex text-sm font-medium text-primary">
          Manage tests →
        </Link>
      </div>
    </div>
