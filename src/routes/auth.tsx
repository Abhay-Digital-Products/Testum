...
export const Route = createFileRoute("/auth")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Sign in or sign up to Testum to access NEET 2027 CBT tests and AI analysis." },
    ],
  }),
  component: AuthPage,
});
...
function AuthPage() {
  ...
  const destination = search?.redirect && search.redirect.startsWith("/") && !search.redirect.startsWith("/auth")
    ? search.redirect
    : "/app";
    ...
    <div className="relative grid min-h-[100dvh] place-items-center overflow-x-hidden bg-background px-4 py-8 sm:py-12">
      ...
      <div className="relative w-full max-w-md rounded-3xl border bg-card p-6 sm:p-8 shadow-elegant">
        ...
        <div className="text-center mb-6">
          <h1 className="font-display text-2xl font-bold tracking-tight">Ready to crack NEET 2027?</h1>
          <p className="mt-1 text-sm text-muted-foreground">Sign in or create your account to continue.</p>
        </div>

        <AuthForm
          defaultTab={search?.tab || "signin"}
        ...
        <div className="mt-6 flex flex-col gap-2 pt-4 border-t text-center">
          ...
          <Link to="/admin-login" className="text-xs font-medium text-muted-foreground hover:text-primary transition-colors">
            Admin login
          </Link>
        </div>
      </div>
