...
export function parseSubjectRanges(
  raw: string,
): Array<{ subject: string; from: number; to: number }> {
...
export function QuickUpload({
  ...
  const { rows: baseRows, duplicates } = useMemo(() => {
    ...
    const rows: Row[] = urls.map((url, i) => {
      ...
      return {
        ...
        url,
        answer: key[n],
        subject: subjectFor(n, ranges, defaultSubject),
        solution: solByN[n],
      };
    });
  ...
  const submit = async () => {
    ...
    if (missingAnswers.length)
      return toast.error(
        `Missing answer for Q${missingAnswers.slice(0, 5).join(", Q")}${missingAnswers.length > 5 ? "…" : ""}`,
      );
    if (duplicates.length)
      return toast.error(`Duplicate question numbers: ${[...new Set(duplicates)].join(", ")}`);
    setBusy(true);
    setProgress(0);
    try {
      if (clashes.length) {
      ...
      const payload = rows.map((r) => ({
        ...
        question_image_url: r.url,
        question_text: null,
        option_type: "text" as const,
      ...
      for (let i = 0; i < payload.length; i += CHUNK) {
        const { error } = await supabase
          .from("questions")
          .insert(payload.slice(i, i + CHUNK) as any);
        if (error) throw error;
            ...
            {STEPS.map((s, i) => {
              ...
              return (
                <div key={s.id} className="flex items-center gap-1.5 flex-1">
                  ...
                  {i < STEPS.length - 1 && (
                    <div
                      className={`h-px flex-1 rounded-full transition-colors ${isDone ? "bg-success/30" : "bg-border"}`}
                    />
                  )}
                </div>
              );
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
          ...
          {step === "questions" && (
            <div className="space-y-3 animate-in fade-in-50 slide-in-from-right-2 duration-200">
              <div className="rounded-xl border bg-secondary/40 p-3 text-xs text-muted-foreground leading-relaxed">
                Paste all question image URLs (any order). The question number is detected from the
                filename —{" "}
                <code className="rounded bg-card border px-1 text-foreground font-mono">
                  /12.jpg
                </code>{" "}
                → Q12. Date-based filenames (e.g.{" "}
                <code className="rounded bg-card border px-1 font-mono">IMG-20260816-…</code>) are
              ...
              {rows.length > 0 && (
                <div className="rounded-xl border bg-card p-3 space-y-2 animate-in fade-in-50">
                  <div className="flex items-center justify-between text-sm font-semibold">
                    <span>
                      {rows.length} questions · {answeredCount} answered
                    </span>
                    <span
                      className={answeredCount === rows.length ? "text-success" : "text-warning"}
                    >
                  ...
                  <div className="h-1.5 rounded-full bg-muted overflow-hidden">
                    <div
                      ...
                      style={{
                        width: rows.length ? `${(answeredCount / rows.length) * 100}%` : "0%",
                      }}
                    />
                  </div>
                  {missingAnswers.length > 0 && (
                    <div className="flex items-start gap-2 text-xs text-warning rounded-lg bg-warning/8 p-2">
                      ...
                      Missing: Q{missingAnswers.slice(0, 15).join(", Q")}
                      {missingAnswers.length > 15 ? ` +${missingAnswers.length - 15} more` : ""}
                    </div>
                  )}
                </div>
              )}
          ...
          {step === "settings" && (
            <div className="space-y-4 animate-in fade-in-50 slide-in-from-right-2 duration-200">
              <div className="rounded-xl border bg-secondary/40 p-3 text-xs text-muted-foreground leading-relaxed">
                Set subject ranges and optional metadata. Leave defaults for NEET PCB (Physics 1-45,
                Chemistry 46-90, Biology 91-180).
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  value={rangesRaw}
                  onChange={(e) => setRangesRaw(e.target.value)}
                  className="font-mono text-xs"
                />
                <p className="text-[11px] text-muted-foreground">
                  Example: <code>physics:1-45, chemistry:46-90, biology:91-180</code>
                </p>
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div className="space-y-1.5">
                  ...
                  />
                  <p className="text-[11px] text-muted-foreground">
                    Only needed if filenames have no numbers
                  </p>
                </div>
              </div>
            ...
            <div className="space-y-4 animate-in fade-in-50 slide-in-from-right-2 duration-200">
              ...
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                {[
                  ...
                  {
                    label: "With Sol.",
                    value: rows.filter((r) => r.solution).length,
                  ...
                  {missingAnswers.length > 0 && (
                    <div className="flex items-start gap-2">
                      ...
                      <span>
                        ...
                        {missingAnswers.length > 12
                          ? ` +${missingAnswers.length - 12} more`
                          : ""}{" "}
                        <span className="font-semibold underline">
                          — click any row below to set the answer
                        </span>
                      </span>
                    </div>
                  )}
              ...
              {clashes.length > 0 && (
                <div className="rounded-xl border border-warning/30 bg-warning/5 p-3 text-xs text-warning">
                  <b>{clashes.length}</b> question{clashes.length > 1 ? "s" : ""} already exist and
                  will be <b>replaced</b>.
                </div>
              )}
                ...
                <div className="flex items-center justify-between border-b bg-secondary/40 px-3 py-2">
                  <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">
                    ...
                    <span className="ml-2 font-normal normal-case text-muted-foreground/70">
                      — click a row to change its answer
                    </span>
                  </span>
                  {rows.length > 0 && (
                    <span className="text-xs text-muted-foreground">
                      Q{rows[0].n} – Q{rows[rows.length - 1].n}
                    </span>
                  )}
                </div>
                  ...
                  {rows.map((r) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <span className="w-9 font-mono text-[11px] font-bold text-muted-foreground shrink-0">
                            Q{r.n}
                          </span>
                          <span
                            className={`w-12 text-[11px] capitalize font-medium shrink-0 ${
                              r.subject === "physics"
                                ? "text-info"
                                : r.subject === "chemistry"
                                  ? "text-success"
                          ...
                          <div className="flex items-center gap-1.5 shrink-0">
                            ...
                            <Pencil
                              className={`h-3.5 w-3.5 transition-colors ${isEditing ? "text-primary" : "text-muted-foreground/40"}`}
                            />
                          </div>
                        </div>
