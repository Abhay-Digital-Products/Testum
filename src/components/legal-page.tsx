...
export function LegalSection({ title, children }: { title: string; children: ReactNode }) {
  return (
    <section className="mt-8">
      ...
      <div className="legal-body mt-2 space-y-2 text-sm leading-relaxed text-muted-foreground">{children}</div>
    </section>
  );
}
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-3xl px-5 pb-20 pt-10">
        ...
        <div className="mt-12 flex flex-wrap gap-3 border-t pt-6 text-sm">
          ...
          <Link to="/refund" className="text-muted-foreground hover:text-primary">Refund Policy</Link>
        </div>
      </main>
    </div>
  );
